...
def main(domains):
    for domain in domains:
        print domain
        print 'Build tfidf model for unshift of {}'.format(domain)

        data = read_sentiment_data(domain)
        generate_tfidf(data, TFIDF_UNSHIFT_PKL_FILE(domain))

if __name__ == '__main__':
    set_args = set(sys.argv[1:])
    if not set_args:
        display_general_help(file_name='tfidf_unshift_model')
        sys.exit()
