...
def read_sentiment_test_data(domain):
    pos = load_files_in_directory(DATA_TEST_FOLDER(domain, 'positive'))
    neg = load_files_in_directory(DATA_TEST_FOLDER(domain, 'negative'))
    return pos, neg
...
def main(domains):
    for domain in domains:
        print domain
        print 'Build classifier model for baseline of {}'.format( domain)
        ...
        data = pos + neg
        tfidf = pickle.load(open(TFIDF_BASE_PKL_FILE(domain), 'rb'))
        tfidf_data = tfidf.transform(data)
        target = generate_label(n_pos, n_neg)
        target_names = ['positive', 'negative']

        train(tfidf_data, target, target_names, CLASSIFIER_BASE_PKL_FILE(domain))
        ...
        test(tfidf_data, target, target_names, CLASSIFIER_BASE_PKL_FILE(domain))
